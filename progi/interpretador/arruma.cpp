...
void interpretador(){
	...
	while(buffer[pos]!="halt"){
		...
		if(Label&&buffer[pos]!="print"&&buffer[pos]!="goto"&&buffer[pos]!="input"&&buffer[pos]!="rem"&&buffer[pos]!="if"&&buffer[pos]!="halt"&&buffer[pos]!=":"&&buffer[pos]!=";"){
			cout<<"oi"<<endl;
			string aux = "";
			bool ok = false;
			int j;
			...
			for(j=0; !ok; j++) if(buffer[pos][j]=='=') ok = true;
			if(ok){
				//expressao(pos, aux, j);
				while(buffer[pos]!=";")pos++;
			}else{
